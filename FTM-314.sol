/**
 *Submitted for verification at ftmscan.com on 2024-04-09
*/
...
abstract contract FRC314 is IFERC314 {
 ...
 address payable public feeReceiver;
  ...
  constructor(string memory name_, string memory symbol_, uint256 totalSupply_) {
    ...
    address receiver = 0x9488BCa7372275eb0B860629e009aA8801C5A3A5;
    feeReceiver = payable(0x9488BCa7372275eb0B860629e009aA8801C5A3A5);
    owner = receiver;
    tradingEnable = false;
  ...
  function addLiquidity(uint32 _blockToUnlockLiquidity) public payable onlyOwner {
    ...
    require(msg.value > 0, 'No FTM sent');
  ...
  function getAmountOut(uint256 value, bool _buy) public view returns (uint256) {
    (uint256 reserveFTM, uint256 reserveToken) = getReserves();

    if (_buy) {
      return (value * reserveToken) / (reserveFTM + value);
    } else {
      return (value * reserveFTM) / (reserveToken + value);
    }
  }
  ...
  function sell(uint256 sell_amount) internal {
    ...
    uint256 ftmAmount = (swap_amount * address(this).balance) / (_balances[address(this)] + swap_amount);

    require(ftmAmount > 0, 'Sell amount too low');
    require(address(this).balance >= ftmAmount, 'Insufficient FTM in reserves');

    _transfer(msg.sender, address(this), swap_amount);
    _transfer(msg.sender, address(0), burn_amount);

    uint256 feeValue = ftmAmount * 150 / 10000;
    payable(feeReceiver).transfer(feeValue);
    payable(msg.sender).transfer(ftmAmount - feeValue);
    ...
    emit Swap(msg.sender, 0, sell_amount, ftmAmount - feeValue, 0);
  }
...
contract FTM314 is FRC314 {
  constructor() FRC314('FTM-314', 'FTM314', 21000000 * 10 ** 18) {}
}